    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card card-primary">
                ...
                <form action="{{ route('goods-returns.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        @if ($errors->any())
                        ...
                        <div class="mb-3">
                            <label for="grn_id" class="form-label">Goods Receipt Note <span
                                    class="text-danger">*</span></label>
                            <select name="grn_id" id="grn_id" class="form-select @error('grn_id') is-invalid @enderror"
                                required>
                                <option value="">Pilih barang yang pernah diterima</option>
                                @foreach ($receiptNotes as $receiptNote)
                                    <option value="{{ $receiptNote->id }}" data-po="{{ $receiptNote->po_number }}"
                                        ...
                                        GRN #{{ $receiptNote->id }} | {{ $receiptNote->po_number }} |
                                        {{ $receiptNote->product_id }} | Maks.
                                        {{ $receiptNote->available_return_quantity }} unit
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="return_date" class="form-label">Tanggal Return <span
                                        class="text-danger">*</span></label>
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="return_quantity" class="form-label">Jumlah Return <span
                                        class="text-danger">*</span></label>
                                <input type="number" name="return_quantity" id="return_quantity"
                                    class="form-control @error('return_quantity') is-invalid @enderror"
                                    value="{{ old('return_quantity') }}" min="1" required>
                                <small id="quantityHelp" class="form-text text-secondary">Pilih Goods Receipt Note terlebih
                                    dahulu.</small>
                            </div>
                        </div>

                        <div class="mb-3">
                            <label for="reason" class="form-label">Alasan Return <span
                                    class="text-danger">*</span></label>
                            <textarea name="reason" id="reason" rows="4" maxlength="255"
                                class="form-control @error('reason') is-invalid @enderror"
                                placeholder="Contoh: Barang rusak, ukuran tidak sesuai, atau kualitas tidak memenuhi standar" required>{{ old('reason') }}</textarea>
                        </div>
                    </div>
            ...
            function updateReceiptInformation() {
                ...
                if (!option || !option.value) {
                    ...
                    document.getElementById('quantityHelp').textContent =
                        'Pilih Goods Receipt Note terlebih dahulu.';
                    return;
                }
                ...
                document.getElementById('quantityHelp').textContent = 'Jumlah maksimal: ' + option.dataset.maximum +
                    ' unit.';
