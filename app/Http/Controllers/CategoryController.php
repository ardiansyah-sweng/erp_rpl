...
class CategoryController extends Controller
{
    ...
    // 2. Filter yang parent_id nya cocok
    $filtered = $allCategories->where('parent_id', $parentId);

    if ($filtered->isEmpty()) {
        return redirect()->back()->with('error', 'Tidak ada kategori dengan parent ID tersebut.');
    }

    // 3. Lempar ke View PDF (pake view yang udah ada: product.category.pdf)
    $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('product.category.pdf', ['categories' => $filtered]);

    // 4. Download/Stream PDF-nya
    return $pdf->stream("kategori_parent_{$parentId}.pdf");
    }
