...
class ItemController extends Controller
{
    ...
    {
        try {
            // 1. Ubah dulu ID acak dari URL jadi ID angka asli
            $itemId = \App\Helpers\EncryptionHelper::decrypt($id);
        } catch (\Exception $e) {
            // Jika gagal dekripsi, pakai ID apa adanya (untuk jaga-jaga)
            $itemId = $id;
        }

        // 2. Cari data di model pakai $itemId yang sudah didekripsi
        $item = (new Item())->getItemById($itemId);

        // 3. Kalau data gak ada, kasih error 404 (biar profesional)
        if (!$item) {
            abort(404);
        }
